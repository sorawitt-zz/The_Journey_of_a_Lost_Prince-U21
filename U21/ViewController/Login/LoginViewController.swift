    ...
    @IBAction func facebookLogin(sender: UIButton) {
        ...
        fbLoginManager.logIn(withReadPermissions: ["public_profile", "email"], from: self) { (result, error) in
            ...
            Auth.auth().signIn(with: credential, completion: { (user, error) in
                ...
                let user = UserModel(uid: userRaw.uid, displayName: userRaw.displayName!, profileImgUrl: "\(userRaw.photoURL!)", email: userRaw.email!)
                //let userValue = ["uid": userRaw.uid, "name": userRaw.displayName!, "avatar": userRaw.photoURL!, "email": userRaw.email!] as [String : Any]
                userRef.child(user.uid).setValue(["uid": userRaw.uid, "name": userRaw.displayName!, "avatar": "\(userRaw.photoURL!)", "email": userRaw.email!])
